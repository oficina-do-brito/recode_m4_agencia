...
public class CarrinhoCompraDAO extends PadraoDao implements IGenericDAO<CarrinhoCompra> {
	...
	public CarrinhoCompra findById(Integer id) {
		...
		try {
			...
			super.rs.first();
			u = new CarrinhoCompra(super.rs.getInt("idCarrinho"), super.rs.getDouble("valorTotal"),
					super.rs.getInt("formaPagamento"), super.rs.getInt("quantItems"), super.rs.getInt("fkCliente"));
			return u;
		} catch (SQLException e) {
			throw new DbIntegrityException(e.getMessage());
		} finally {
	...
	public ArrayList<CarrinhoCompra> findAll() {
		...
		try {
			...
			while (super.rs.next()) {
				CarrinhoCompra u = new CarrinhoCompra(super.rs.getInt("idCarrinho"), super.rs.getDouble("valorTotal"),
						super.rs.getInt("formaPagamento"), super.rs.getInt("quantItems"), super.rs.getInt("fkCliente"));
				usuarios.add(u);
			}
			return usuarios;
